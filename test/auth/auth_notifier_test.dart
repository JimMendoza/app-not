...
void main() {
  group('AuthNotifier', () {
    test('login persiste token y devuelve usuario canonico de /app/me', () async {
      ...
      final InMemoryKeyValueStorageService storage =
          InMemoryKeyValueStorageService(
            seed: <String, Object?>{
              SessionStorageKeys.rememberSession: '0',
            },
          );
      ...
      expect(storage.snapshot[SessionStorageKeys.accessToken], loginUser.token);
      expect(storage.snapshot[SessionStorageKeys.tokenType], loginUser.tokenType);
      expect(storage.snapshot[SessionStorageKeys.rememberSession], '1');
      expect(container.read(authProvider).authStatus, AuthStatus.authenticated);
      expect(
    ...
    test('checkAuthStatus restaura sesion autenticada y consentimiento', () async {
      ...
      final InMemoryKeyValueStorageService storage =
          InMemoryKeyValueStorageService(
            seed: <String, Object?>{
              ...
              SessionStorageKeys.dataPolicyAcceptanceKey(canonicalUser.username):
    ...
    test('logout limpia sesion local e invalida deviceId push si existe', () async {
      final canonicalUser = buildCanonicalUser();
      final FakeAuthRepository repository = FakeAuthRepository(
        currentUserResponse: canonicalUser,
      );
      ...
      expect(container.read(authProvider).authStatus, AuthStatus.notAuthenticated);
      expect(storage.snapshot.containsKey(SessionStorageKeys.accessToken), isFalse);
      expect(storage.snapshot.containsKey(SessionStorageKeys.tokenType), isFalse);
